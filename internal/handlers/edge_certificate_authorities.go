...
import (
	...
	ca "github.com/openziti/ziti-mcp-server/internal/gen/edge/client/certificate_authority"
		...
		return client.WithAuthenticatedClient(req, cfg, "list CAs", s,
			func(httpClient *http.Client, _ string) (any, error) {
				...
				return fetchAllPages(func(limit, offset int64) (map[string]any, error) {
					resp, err := ec.CertificateAuthority.ListCas(
						ca.NewListCasParams().WithLimit(&limit).WithOffset(&offset), noAuth)
					if err != nil {
						return nil, err
					}
