...
import (
	...
	fterminator "github.com/openziti/ziti-mcp-server/internal/gen/fabric/client/terminator"
		...
		return client.WithAuthenticatedClient(req, cfg, "list fabric terminators", s,
			func(httpClient *http.Client, _ string) (any, error) {
				...
				return fetchAllPages(func(limit, offset int64) (map[string]any, error) {
					resp, err := fc.Terminator.ListTerminators(
						fterminator.NewListTerminatorsParams().WithLimit(&limit).WithOffset(&offset))
					if err != nil {
						return nil, err
					}
