...
import (
	...
	service_edge_router_policy "github.com/openziti/ziti-mcp-server/internal/gen/edge/client/service_edge_router_policy"
		...
		return client.WithAuthenticatedClient(req, cfg, "list service edge router policies", s,
			func(httpClient *http.Client, _ string) (any, error) {
				...
				return fetchAllPages(func(limit, offset int64) (map[string]any, error) {
					resp, err := ec.ServiceEdgeRouterPolicy.ListServiceEdgeRouterPolicies(
						service_edge_router_policy.NewListServiceEdgeRouterPoliciesParams().WithLimit(&limit).WithOffset(&offset), noAuth)
					if err != nil {
						return nil, err
					}
