		...
		return client.WithAuthenticatedClient(req, cfg, "list current identity authenticators", s,
			func(httpClient *http.Client, _ string) (any, error) {
				...
				return fetchAllPages(func(limit, offset int64) (map[string]any, error) {
					resp, err := ec.CurrentAPISession.ListCurrentIdentityAuthenticators(
						currentapisession.NewListCurrentIdentityAuthenticatorsParams().WithLimit(&limit).WithOffset(&offset), noAuth)
					if err != nil {
						return nil, err
					}
