...
import (
	...
	authpolicy "github.com/openziti/ziti-mcp-server-go/internal/gen/edge/client/auth_policy"
	...
	r.Register(tools.ToolDef{
		...
		return client.WithAuthenticatedClient(req, cfg, "list auth policies", s,
			func(httpClient *http.Client, _ string) (any, error) {
				...
				resp, err := ec.AuthPolicy.ListAuthPolicies(authpolicy.NewListAuthPoliciesParams(), noAuth)
				if err != nil {
					return nil, err
				}
